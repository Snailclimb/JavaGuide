...
According to the inter-stage dependency theory mentioned earlier, when we execute the `mvn test` command, all stages from validate to test will be executed, which explains why the project code can be automatically compiled when testing is executed.

### clean life cycle

The purpose of the clean life cycle is to clean the project and consists of 3 stages:
...
```xml
<phases>
  <!-- Perform some work that needs to be completed before cleaning -->
  <phase>pre-clean</phase>
  <!-- Remove all files generated by the previous build -->
  <phase>clean</phase>
  <!-- Perform some work that needs to be completed immediately after clean -->
...
According to the inter-stage dependency theory mentioned earlier, when we execute `mvn clean`, the pre-clean and clean phases in the clean life cycle will be executed.

### site life cycle

The purpose of the site life cycle is to build and publish the project site, which consists of 4 stages:
...
```xml
<phases>
  <!-- Perform some work that needs to be done before generating site documentation -->
  <phase>pre-site</phase>
  <!-- Generate site documentation for the project -->
  <phase>site</phase>
  <!-- Perform some work that needs to be done after generating the site documentation and prepare for deployment -->
  <phase>post-site</phase>
  <!-- Deploy the generated site documents to a specific server -->
...
Maven can automatically generate a friendly site based on the information contained in `pom.xml` to facilitate team communication and release of project information.

## Maven plugin

Maven is essentially a plug-in execution framework, and all execution processes are completed independently by plug-ins. Commands such as install, clean, and deploy that we use daily are actually Maven plug-ins at the bottom level. For information about Maven’s core plug-ins, please refer to this official document: <https://maven.apache.org/plugins/index.html>.

Local default plugin path: `${user.home}/.m2/repository/org/apache/maven/plugins`

![](https://oss.javaguide.cn/github/javaguide/tools/maven/maven-plugins.png)

In addition to the plug-ins that come with Maven, there are also plug-ins provided by third parties, such as the single test coverage plug-in jacoco-maven-plugin, the plug-in that helps develop and detect non-standard places in the code, maven-checkstyle-plugin, and the sonar-maven-plugin that analyzes code quality. Moreover, we can also customize plug-ins to meet our own needs.
...
You can understand the Maven plug-in as a collection of tasks. Users can directly run the tasks of the specified plug-in through the command line, or they can mount the plug-in tasks to the build life cycle and run them along with the life cycle.

Maven plug-ins are divided into the following two types:

- **Build plugins**: Executed at build time.
- **Reporting plugins**: executed during website generation.

## Maven multi-module management

Multi-module management simply means dividing a project into multiple modules, each module is only responsible for a single function implementation. The intuitive performance is that there is more than one `pom.xml` file in a Maven project, and there will be multiple `pom.xml` files in different directories, thereby achieving multi-module management.

In addition to making project development and management easier, multi-module management also has the following benefits:

1. Reduce the coupling between codes (from class-level coupling to jar package-level coupling);
2. Reduce duplication and improve reusability;
3. Each module can be self-explanatory (through module name or module documentation);
4. Modules also standardize the division of code boundaries, making it easy for developers to determine the content they are responsible for through modules.

Under multi-module management, there will be a parent module and the others are sub-modules. The parent module usually has only one `pom.xml` and nothing else. The `pom.xml` of the parent module generally only defines the version number of each dependency, which sub-modules are included, and which plug-ins are included. However, it should be noted that if the dependency is only used in a certain sub-project, it can be introduced directly in the pom.xml of the sub-project to prevent the parent pom from being too bloated.

As shown in the figure below, the Dubbo project is divided into multiple sub-modules such as dubbo-common (common logic module), dubbo-remoting (remote communication module), and dubbo-rpc (remote calling module).

![](https://oss.javaguide.cn/github/javaguide/tools/maven/dubbo-maven-multi-module.png)

## Article recommendation

- [Security classmates talk about the arbitration mechanism of Maven indirect dependency scenarios - Alibaba Developers - 2022](https://mp.weixin.qq.com/s/flniMiP-eu3JSBnswfd_Ew)
- [Efficient use of Java build tools | Maven - Alibaba Developers - 2022](https://mp.weixin.qq.com/s/Wvq7t2FC58jaCh4UFJ6GGQ)
- [Safety classmates tell the story of Maven repackaging - Alibaba Developers - 2022](https://mp.weixin.qq.com/s/xsJkB0onUkakrVH0wejcIg)

## Reference

- "Maven Combat"
- Introduction to Repositories - Maven official documentation: <https://maven.apache.org/guides/introduction/introduction-to-repositories.html>
- Introduction to the Build Lifecycle - Maven official documentation: <https://maven.apache.org/guides/introduction/introduction-to-the-lifecycle.html#Lifecycle_Reference>
- Maven dependency scope: <http://www.mvnbook.com/maven-dependency.html>
- To resolve maven dependency conflicts, this article is enough! ：<https://www.cnblogs.com/qdhxhz/p/16363532.html>
