...
Asymmetric encryption algorithm refers to an algorithm that uses different keys for encryption and decryption. It is also called a public key encryption algorithm. These two keys are different from each other, one is called the public key and the other is called the private key. The public key can be made public for anyone to use, while the private key must be kept secret.

If you use a public key to encrypt data, you can only use the corresponding private key to decrypt (encrypt) it; if you use a private key to encrypt data, you can only use the corresponding public key to decrypt (sign) it. This enables secure data transmission and identity authentication.
...
The RSA (Rivest–Shamir–Adleman algorithm) algorithm is an asymmetric encryption algorithm based on the difficulty of factoring large numbers. It requires selecting two large prime numbers as part of the private key, and then calculating their product as part of the public key (seeking two large prime numbers is relatively simple, but factoring their product is extremely difficult). For a detailed introduction to the principles of the RSA algorithm, please refer to this article: [Do you really understand the RSA encryption algorithm? - Brother Xiao Fu](https://www.cnblogs.com/xiaofuge/p/16954187.html).

The security of the RSA algorithm depends on the difficulty of large number decomposition. Currently, 512-bit and 768-bit RSA public keys have been successfully decomposed, so it is recommended to use a key length of 2048 bits or above.

The advantage of the RSA algorithm is that it is simple and easy to use and can be used for data encryption and digital signatures; the disadvantage is that the operation speed is slow and it is not suitable for the encryption of large amounts of data.

The RSA algorithm is currently the most widely used asymmetric encryption algorithm. The RSA algorithm is used in protocols such as SSL/TLS and SSH.

![SHA-256 with RSA encryption in HTTPS certificate signature algorithm](https://oss.javaguide.cn/github/javaguide/system-design/security/encryption-algorithms/https-rsa-sha-256.png)

Code example of implementing RSA algorithm based on Java:
...
```bash
Original String: Java learning + interview guide: javaguide.cn
RSA Encrypted Data: T9ey/CEPUAhZm4UJjuVNIg8RPd1fQ32S9w6+rvOKxmuMumkJY2daFfWuCn8A73Mk5bL6TigOJI0GHfKOt/W2x9 68qLM3pBGCcPX17n4pR43f32IIIz9iPdgF/INOqDxP5ZAtCDvTiuzcSgDHXqiBSK5TDjtj7xoGjfudYAXICa8p WitnqDgJYoo2J0F8mKzxoi8D8eLE455MEx8ZT1s7FUD/z7/H8CfShLRbO9zq/zFI06TXn123ufg+F4lDaq/5ja IxGVEUB/NFeX4N6OZCFHtAV32mw71BYUadzI9TgvkkUr1rSKmQ0icNhnRdKedJokGUh8g9QQ768KERu92Ibg==
RSA Decrypted Data: Java Learning + Interview Guide: javaguide.cn
```

### DSA

The DSA (Digital Signature Algorithm) algorithm is an asymmetric encryption algorithm based on the difficulty of discrete logarithms. It requires selecting a prime number q and a multiple of q p as part of the private key, and then calculating a primitive root g modulo p and an integer y modulo q as part of the public key. The security of the DSA algorithm depends on the difficulty of the discrete logarithm. Currently, a 1024-bit DSA public key has been successfully cracked, so it is recommended to use a key length of 2048 bits or above.

The advantage of the DSA algorithm is that the digital signature is fast and suitable for generating digital certificates; the disadvantage is that it cannot be used for data encryption, and the signature process requires random numbers.

DSA algorithm signature process:

1. Use the message digest algorithm to encrypt the data to be sent and generate a message digest, which is a short, unique, irreversible data representation.
2. The sender uses its own DSA private key to encrypt the message digest to form a digital signature, which is a data append that can prove the source and integrity of the data.
3. Send the original data together with the digital signature to the recipient via the Internet.
4. The receiver uses the sender’s public key to decrypt the digital signature and obtain the information digest. At the same time, the receiver also uses the message digest algorithm to encrypt the received original data to obtain another information digest. The receiver compares the two information digests. If the two are consistent, it means that the data has not been tampered with or damaged during the transmission process; otherwise, it means that the data has lost security and confidentiality.
...
## SummaryThis article introduces three encryption algorithms: hashing algorithm, symmetric encryption algorithm and asymmetric encryption algorithm.

- The hash algorithm is a technology that uses mathematical methods to generate a fixed-length unique identifier for data, which can be used to verify the integrity and consistency of the data. Common hash algorithms include MD, SHA, MAC, etc.
- Symmetric encryption algorithm is an algorithm that uses the same key for encryption and decryption. It can be used to protect the security and confidentiality of data. Common symmetric encryption algorithms include DES, 3DES, AES, etc.
- The asymmetric encryption algorithm is an algorithm that uses different keys for encryption and decryption. It can be used to achieve secure transmission of data and identity authentication. Common asymmetric encryption algorithms include RSA, DSA, ECC, etc.

## Reference

- In-depth understanding of perfect hashing - Tencent Technology Engineering: <https://mp.weixin.qq.com/s/M8Wcj8sZ7UF1CMr887Puog>
- Practical cryptography for developers (2) - Hash function: <https://thiscute.world/posts/practical-cryptography-basics-2-hash/>
- Wonderful DSA algorithm for safe travel: <https://zhuanlan.zhihu.com/p/347025157>
