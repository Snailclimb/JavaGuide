...
- java中，基本数据类型，使用的是值传递的方式。直接复制值的副本到形参中使用。
- String可以理解为“值传递”，实际还是引用传递，但是由于String 内部使用final修饰，导致已创建的String无法修改，从而新建一个String副本进行传值。
- 引用数据类型，例如数组和集合等，都是属于引用传递。将引用的key进行副本复制，然后执行方法，通过引用修改值。相当于复制钥匙，来操作同一个房间。
