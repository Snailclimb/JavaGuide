<template>
  <button
    ...
    :title="isHidden ? 'Thoát chế độ đọc tập trung' : 'Chế độ đọc tập trung'"
    @click="toggleLayout"
  >
    <svg
    ...
    <span class="btn-text">{{
      isHidden ? "Thoát tập trung" : "Đọc tập trung"
    }}</span>
  </button>
</template>
...
const IMMERSIVE_TITLE = "JavaGuide - Đang ở chế độ đọc tập trung";

// Tính tiêu đề gốc của trang hiện tại
...
// Theo dõi thay đổi trạng thái chế độ đọc tập trung
watch(isHidden, (newVal) => {
  applyHiddenState(newVal);
  localStorage?.setItem(STORAGE_KEY, String(newVal));
});

// Theo dõi việc chuyển trang để cập nhật tiêu đề
watch(
  () => pageData.value.path,
  () => {
    if (isHidden.value) {
