...
![Redis 事务](/oss/github/javaguide/database/redis/redis-transactions.png)
...
![Redis 为什么不支持回滚](/oss/github/javaguide/database/redis/redis-rollback.png)
...
![](/oss/github/javaguide/database/redis/redis-pipeline-vs-transaction.png)
...
![bigkey 判定标准](/oss/github/javaguide/database/redis/bigkey-criterion.png)
...
![阿里云Key分析](/oss/github/javaguide/database/redis/aliyun-key-analysis.png)
...
![京东零售开源的 hotkey](/oss/github/javaguide/database/redis/jd-hotkey.png)
...
![通过阿里云的Proxy Query Cache优化热点Key问题](/oss/github/javaguide/database/redis/aliyun-hotkey-proxy-query-cache.png)
...
![缓存穿透](/oss/github/javaguide/database/redis/redis-cache-penetration.png)
...
![Bloom Filter 的简单原理示意图](/oss/github/javaguide/cs-basics/algorithms/bloom-filter-simple-schematic-diagram.png)

Bloom Filter 会使用一个较大的 bit 数组来保存所有的数据，数组中的每个元素都只占用 1 bit ，并且每个元素只能是 0 或者 1（代表 false 或者 true），这也是 Bloom Filter 节省内存的核心所在。这样来算的话，申请一个 100w 个元素的位数组只占用 1000000Bit / 8 = 125000 Byte = 125000/1024 KB ≈ 122KB 的空间。

![位数组](/oss/github/javaguide/cs-basics/algorithms/bloom-filter-bit-table.png)

具体是这样做的：把所有可能存在的请求的值都存放在布隆过滤器中，当用户请求过来，先判断用户发来的请求的值是否存在于布隆过滤器中。不存在的话，直接返回请求参数错误信息给客户端，存在的话才会走下面的流程。

加入布隆过滤器之后的缓存处理流程图如下：

![加入布隆过滤器之后的缓存处理流程图](/oss/github/javaguide/database/redis/redis-cache-penetration-bloom-filter.png)
...
![缓存击穿](/oss/github/javaguide/database/redis/redis-cache-breakdown.png)
...
![缓存雪崩](/oss/github/javaguide/database/redis/redis-cache-avalanche.png)
