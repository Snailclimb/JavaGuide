...
“你这个 Agent，每一步都调 Claude Opus 4.7？”

我点点头：“对啊，肯定得用地表最强的啊，效果好。”

他沉默了两秒。

“那意图分类、标题生成、JSON 修复、简单摘要，也全走 Opus？”
...
| -------- | ------------------------------------ | ---------------------------------------------------- |
| 核心职责 | 模型选择                             | 统一接入、路由、限流、fallback、观测、成本治理       |
...
可以这么理解：**Router 是 Gateway 里的一个核心模块，但 Gateway 不等于 Router**。
...
如果 Gateway 只是在同机房里做一次内存路由、Token 估算和日志写入，额外延迟通常不是主要矛盾。真正吃时间的是模型排队、长上下文推理、跨区域网络、输出 Token 过多、工具调用和重试。
...
工程里最怕的不是第一版简单，而是简单到没有边界。你可以先不做完整 Gateway，但最好从第一天就别让模型调用散落在所有业务代码里。
...
这其实是不太可取的。对高价值、强推理、高风险任务，强模型确实值得。但如果所有请求都走最强模型，你很快会遇到三个问题：

1. **成本不可控**：分类、改写、摘要这类任务也走旗舰模型，单次看不贵，流量上来后很吓人。就算是再不差钱，该省也得省省吧？
...
这就是 Gateway 的价值：**不是固定选一个最强模型，而是在质量、成本、延迟之间做动态取舍**。
...
比如固定规则过滤、关键词判断、权限校验、简单模板填充，这些交给代码更稳定。别让模型判断“用户是不是空字符串”“文件后缀是不是 PDF”。

**第二层：能用小模型就先用小模型。**

典型是意图分类、轻量摘要、标题生成、简单改写、低风险信息抽取。这里要做的是结构化输出、枚举约束和失败兜底，而不是一上来就用旗舰模型。

**第三层：复杂任务再升级。**

多文档归纳、代码架构设计、复杂 Agent 规划、强事实核验、金融法务医疗相关内容，这类任务本身错误成本高，强模型更合理。

### LLM Router 的底层决策逻辑

LLM Router 的任务，是给每个请求选一个合适模型。这个“合适”通常不是单一维度，而是质量、成本、延迟、上下文窗口、风险策略的综合结果。

`ulab-uiuc/LLMRouter` 这个开源项目把路由策略做得很丰富，包括 KNN、SVM、MLP、Matrix Factorization、Elo Rating、Graph Router、多轮 Router、个性化 Router、Agentic Router 等。它的价值不在于让每个业务系统都把这些算法全上，而是提醒我们一件事：**模型路由本身可以从简单规则逐步演进成可训练、可评估、可迭代的系统**。
...
很多团队真正缺的不是 GNN 路由，而是下面这些基础能力：每个请求有没有场景标签？有没有质量反馈？有没有成本记录？有没有失败样本？有没有模型升级后的对比数据？

这些没有，学习型 Router 只会把复杂度提前。

## LLM Gateway 需要具备哪些能力？

### 多模型统一接入：先把模型调用收口

业务代码里最不应该到处散落的，就是供应商 SDK 调用。

今天一个服务调 OpenAI，明天另一个服务调 DeepSeek，后天一个定时任务又接了 Gemini。短期看都能跑，长期会变成一堆重复逻辑：API Key、超时、重试、流式解析、错误码、usage、日志格式、模型名映射，每个地方都处理一遍。
...
别在一开始追求“大而全”的平台。先把模型调用收口，收益已经很明显。

### 模型路由：按场景、成本、延迟和风险选模型

模型路由是 LLM Gateway 最容易产生收益的能力之一。
...
这段代码的重点不是复杂，而是边界清楚：路由器只负责选模型，不负责调模型；健康检查只提供状态，不掺业务逻辑；预算判断独立出来，方便后续替换估算方式。
...
第二，fallback 不能偷偷改变业务语义。法务审核任务从强模型降到便宜模型，如果不标记、不审核，很容易把风险藏起来。高风险场景里，降级可以返回“当前系统繁忙，稍后重试”，不一定要硬给一个低质量答案。
...
Cloudflare AI Gateway、LiteLLM、Kong AI Gateway 这类产品都把日志、Token、成本、错误、延迟、缓存、限流当成主要能力。原因很简单：AI 应用出问题时，如果只记录最终答案，基本没法复盘。
...
如果让我从零设计一个 LLM Gateway，我会先拆成这些组件：
...
这样做的好处是每一步都有收益，不会一上来就把自己拖进平台工程。
...
这里最关键的是评测数据。没有评测集、没有线上 trace、没有人工或自动评分，所谓智能路由很容易变成“看起来聪明，实际上不可控”。

### 路由错了怎么办？

路由一定会错。

问题不是能不能避免所有错误，而是错了之后能不能发现、能不能兜底、能不能复盘。
...
现在 LLM Gateway / Router 方案很多，别只看“支持多少模型”。选型时要看团队技术栈、合规要求、流量规模、是否要自托管、是否已有 API 网关、是否需要深度观测。
...
如果企业已有 Kong 基础设施，考虑 Kong AI Gateway。优势不是“更 AI”，而是把 AI 流量放进已有 API 治理体系里。

如果已经重度使用 Cloudflare，可以用 Cloudflare AI Gateway 先把观测、缓存、限流和统一入口补上。

如果你真正想做智能路由，先准备评测集和线上 trace，再谈 LLMRouter 这类学习型策略。没有数据，路由算法越复杂，越难解释。

一句话：**先解决工程治理，再追求智能路由**。

## 怎么衡量 LLM Gateway 做得好不好？

LLM Gateway 做得好不好，不能只看“接了多少模型”。模型多不代表系统稳。
...
所以路由规则要像 Prompt 一样版本化，像代码一样回归测试。

## 总结

LLM Gateway 不是“多包一层代理”，它要做的是把模型调用变成可治理的基础设施。

LLM Gateway 是模型调用的控制面，不只是转发请求。**LLM Router 是 Gateway 的核心模块之一**，但 Gateway 还要管限流、fallback、观测、成本、审计和安全。

AI 应用越接近生产，Gateway 越像基础设施。早期你可以不做得很重，但模型调用一定要收口。等成本、限流、供应商切换、审计、质量回放一起压上来时，再补这层就会痛很多。
