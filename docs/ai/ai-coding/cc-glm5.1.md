...
![点击添加模型](/oss/ai/coding/glm5.1-cc/add-model-entry.png)

选择对应的模型：

![选择模型](/oss/ai/coding/glm5.1-cc/select-model.png)

配置参数：

![配置参数](/oss/ai/coding/glm5.1-cc/config-params.png)

Claude Code 内部模型环境变量与目标模型对应关系的 JSON 配置：

![Claude Code 内部模型环境变量与模型对应关系 JSON 配置](/oss/ai/coding/glm5.1-cc/model-env-json-config.png)

如果你更偏向页面开发，推荐通过 VSCode + Claude Code for VS Code 方式进行交互和编码验收。完成插件安装之后，可以直接在 IDE 中与模型对话和代码审查，相对于 CLI 界面会更直观一些：

![VSCode + Claude Code for VS Code](/oss/ai/coding/glm5.1-cc/vscode-claude-code.png)
...
![AI 自主完成技术方案规划](/oss/ai/coding/glm5.1-cc/ai-tech-plan.png)

AI 结合需求，针对 Agent 拆解出技术选型和 Arthas 集成方案的检索。从检索关键字可以看出，它在方案选取上优先考虑成熟稳定的解决方案：

![AI 检索 Agent 技术选型和 Arthas 集成方案](/oss/ai/coding/glm5.1-cc/agent-arthas-integration-research.png)
...
![AI 输出的系统架构设计图](/oss/ai/coding/glm5.1-cc/system-architecture-design.png)
...
![AI 输出的核心角色分工表](/oss/ai/coding/glm5.1-cc/core-component-roles.png)
...
![AI 输出的数据流设计](/oss/ai/coding/glm5.1-cc/data-flow-design.png)
...
![AI 给出的后续扩展建议](/oss/ai/coding/glm5.1-cc/extension-suggestions.png)
...
![AI 自主编码过程](/oss/ai/coding/glm5.1-cc/ai-coding-process.png)
...
![AI 完成编码后输出的交付清单](/oss/ai/coding/glm5.1-cc/delivery-checklist.png)
...
![AI 完成 Agent Chat UI 页面集成](/oss/ai/coding/glm5.1-cc/agent-chat-ui-integration.png)
...
![Agent 诊断效果演示](/oss/ai/coding/glm5.1-cc/agent-diagnosis-demo.png)
...
![搜索接口耗时 18 秒的调测结果](/oss/ai/coding/glm5.1-cc/search-api-18s-result.png)
...
![AI 给出的慢查询解决方案](/oss/ai/coding/glm5.1-cc/slow-query-solution.png)

同时给出了分阶段优化建议和预期效果：

![AI 给出的分阶段优化建议](/oss/ai/coding/glm5.1-cc/phased-optimization-suggestions.png)
...
![AI 输出的索引优化 SQL 脚本](/oss/ai/coding/glm5.1-cc/index-optimization-sql.png)

从代码 diff 可以直观地看到，AI 在既有代码中进行增量迭代，将`LIKE`模糊查询替换为全文检索，同时保留原有业务逻辑不变：

![AI 在既有代码中完成增量优化](/oss/ai/coding/glm5.1-cc/incremental-code-optimization.png)
...
![AI 针对深分页场景基于阈值自动切换查询策略的代码实现](/oss/ai/coding/glm5.1-cc/deep-pagination-threshold-code.png)

全部优化完成后，AI 输出了最终的优化效果总结，涵盖各接口的优化前后对比：

![AI 输出的最终优化效果总结](/oss/ai/coding/glm5.1-cc/optimization-summary.png)

### 优化效果验证

完成改造后再次对接口进行压测，效果如下。接口经过预热后耗时稳定控制在 300ms 以内，**从 18375ms 降至 300ms 以内，性能提升超过 60 倍。** 整个过程中，笔者做的事情就三件：给出问题、评审方案、验收结果。

![优化后接口耗时降至 300ms 以内](/oss/ai/coding/glm5.1-cc/optimized-api-300ms.png)
