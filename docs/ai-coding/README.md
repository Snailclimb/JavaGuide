...
2. [AI 编程选 CLI 还是 IDE？](./practices/cli-vs-ide.md)：再把工具路线分清楚，别一上来就陷入工具名之争。
...
4. [OpenAI Codex 最佳实践指南](./practices/codex-best-practices.md)、[AI 编程必备 Skills 推荐](./practices/programmer-essential-skills.md)：想把自动化、代码审查和 Skills 玩起来，再看这两篇。
5. [Spec Coding 规范驱动编程](./practices/spec-coding.md)、[Vibe Coding 实用技巧总结](./practices/the-cool-tricks-for-vibe-coding.md)：掌握规范驱动的开发流程，再系统梳理 Vibe Coding 的实战技巧。
6. 工具栈确定后，再按需看 Qoder、Trae、DeepSeek V4 + Claude Code、Claude Code 接入第三方模型等实战案例（均位于 `cases/` 目录下）。
