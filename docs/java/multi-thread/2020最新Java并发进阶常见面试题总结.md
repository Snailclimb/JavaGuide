...
注意：这里我怀疑volatile关键字是可有可无的，而且也并不能阻止引用类型的变量赋值时候的指令重排。volatile修饰基本类型的时候可以保证可见性，修饰引用类型的时候，只能保证在其引用赋值的时候不发生指令重排，不能保证new对象的时候，这个new的过程不指令重排。所以我理解的来说volatile并没有起到上述作用，但是我还没有找到证据，以后有找到证据再来补充。但是从volatile实现原理上来说是不可能起到这个作用的。
