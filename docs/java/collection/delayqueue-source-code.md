...
![延迟任务](/oss/github/javaguide/java/collection/delayed-task.png)
...
![DelayQueue类图](/oss/github/javaguide/java/collection/delayqueue-class-diagram.png)
...
![](/oss/github/javaguide/java/collection/delayqueue-take-0.png)
...
![](/oss/github/javaguide/java/collection/delayqueue-take-1.png)

注意，调用 `await` 之后 t1 就会释放 `lcok` 锁，假如 `DelayQueue` 持续为空，那么 t2、t3 也会像 t1 一样执行相同的逻辑并进入 `conditionWaiter` 队列中。

![](/oss/github/javaguide/java/collection/delayqueue-take-2.png)
