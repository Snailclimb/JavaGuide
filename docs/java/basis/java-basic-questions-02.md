...
- **生存时间** ：从变量在内存中的生存时间上看，成员变量是对象的一部分，它随着对象的创建而存在，而局部变量随着方法的调用而产生，方法的结束而消亡。
...
构造方法是一种特殊的方法，主要作用是完成对象的初始化工作（创建一个新的对象实例）。
...
overload：load是装载的意思，一辆卡车可以装载很多的货物。

override：ride是骑的意思，一个人只可以开一辆卡车送货，可以换一辆新的、容量更大的卡车送货。
