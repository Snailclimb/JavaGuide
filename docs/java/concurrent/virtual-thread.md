...
平台线程依赖操作系统调度。虚拟线程由 JDK 自己的调度器调度，再由平台线程承载执行。JEP 444 中说明，虚拟线程调度器是一个 FIFO 模式的 `ForkJoinPool`，它和并行流使用的 common pool 不是同一个池。
...
如果只是传递请求上下文、用户 ID、Trace ID，通常问题不大。如果是缓存数据库连接、大数组、复杂 formatter、客户端对象，就要重新评估。新版本 Java 中的 Scoped Values 也值得关注，它更适合在大量虚拟线程之间传递不可变上下文。
...
它会在虚拟线程阻塞且被固定时打印调用栈，适合本地或测试环境定位问题。JDK 24 的 JEP 491 之后，这个参数已经不再需要，设置也不会产生效果。
...
它们都属于轻量级并发的思路，但 Java 虚拟线程是 `java.lang.Thread` 的实现，纳入了 Java 原有线程模型。业务代码不需要写 `async/await`，也不需要手动 yield。对开发者来说，它更像“便宜很多的线程”。
