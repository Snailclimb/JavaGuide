...
> **CAS的原子性** 是由底层硬件指令直接保证的，操作系统和编程语言通过封装指令提供API：现代CPU架构（如x86的CMPXCHG指令、ARM的LDREX/STREX指令）提供了原子性的CAS操作指令，Java语言通过`sun.misc.Unsafe`类调用本地方法（Native Method），最终映射到CPU指令
