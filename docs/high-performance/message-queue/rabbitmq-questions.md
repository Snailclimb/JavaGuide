...
description: 本文总结 RabbitMQ 常见面试题与核心知识点，涵盖 AMQP 协议、Exchange 交换机类型（Direct/Topic/Fanout）、消息确认机制、死信队列、延迟队列、优先级队列、Quorum Queue、Streams 等，助力 RabbitMQ 学习与面试准备。
category: 高性能
tag:
  - 消息队列
head:
...
本文已针对 RabbitMQ 4.x 进行更新，明确标注各特性的版本依赖，特别强调了镜像队列（已移除）、Quorum Queue（高可靠队列优先考虑）和 Streams（3.9+）的选型差异。
...
- **集群模式**：
  ...
  - **Quorum Queue**（3.8+ 推荐）：基于 Raft 协议，支持更严格的仲裁写入（N/2 + 1）
...
> - **3.13**：镜像队列仍可用但已废弃
> - **4.0+**：镜像队列**完全移除**，需要复制和高可用队列时优先考虑 Quorum Queue 或 Streams
>
...
**Quorum Queue**（3.8+ 推荐，高可靠队列优先考虑）

基于 Raft 协议的复制队列，是 RabbitMQ 3.8+ 推荐的高可用方案。对于需要复制和高可用的队列，应该优先考虑 Quorum Queue：
...
Quorum Queue 不适合所有场景。临时队列、高频创建删除队列、极低延迟场景、超大规模积压和大规模 fanout，更应该优先评估 Classic Queue 或 Streams。简单说：需要复制和高可用时优先考虑 Quorum Queue；需要反复读取、事件溯源或长时间保留时优先评估 Streams。
...
| **4.0+**   | 镜像队列完全移除                        | 高可靠队列优先考虑 Quorum Queue 或 Streams |
