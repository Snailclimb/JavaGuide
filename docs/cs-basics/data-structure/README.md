---
title: 数据结构专题：面试复习路线、复杂度、Java 集合与工程场景
description: 数据结构面试复习路线，涵盖数组、链表、栈、队列、哈希表、树、图、堆、Trie、并查集、跳表、红黑树、布隆过滤器、LRU 和 Java 后端工程场景。
category: 计算机基础
tag:
  - 数据结构
  - 算法
...
head:
  - - meta
    - name: keywords
      content: 数据结构,数据结构面试题,数组,链表,栈,队列,哈希表,树,图,堆,Trie,并查集,跳表,红黑树,布隆过滤器,LRU,Java集合,后端面试
---

这份 **数据结构专题** 按面试和 Java 后端场景组织内容：先理解数据怎么存，再看常见操作复杂度，最后把结构和 Java 集合、MySQL 索引、Redis、缓存、消息队列这些工程问题连起来。
