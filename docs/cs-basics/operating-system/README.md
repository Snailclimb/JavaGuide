...
5. [虚拟内存详解：地址转换、TLB、缺页中断与页面置换](./virtual-memory.md)：把分页、页表、TLB、缺页中断和页面置换串起来。
6. [I/O 多路复用详解：select、poll、epoll 原理与区别](./io-multiplexing.md)：理解一个线程处理海量连接背后的内核机制。
7. [零拷贝详解：mmap、sendfile 与 splice](./zero-copy.md)：搞清传统 I/O、mmap、sendfile、splice 的拷贝路径和适用场景。
8. [Linux 基础知识总结](./linux-intro.md)：掌握目录结构、文件权限、常用命令和基础排障能力。
9. [Shell 编程基础知识总结](./shell-intro.md)：学习变量、条件、循环、函数和常用脚本写法。
