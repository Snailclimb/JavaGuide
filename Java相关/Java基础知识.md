...
1 枚举类的实例是有限的，例如季节类，只有春夏秋冬
2 在java中，enum关键字定义枚举类，其地位与class、interface相同
3 枚举类是一种特殊的类，有自己的成员变量、成员方法、构造器（只能使用 private 访问修饰符，所以无法从外部调用构造器，构造器只在构造枚举值时被调用）

## 35 泛型的用法
[泛型初级--参考博客](https://blog.csdn.net/s10461/article/details/53941091)
[泛型进阶--博客](https://blog.csdn.net/ShuSheng0007/article/details/80720406)
泛型：参数化类型，也就是说所操作的数据类型被指定为一个参数。泛型只是在编译期有效，不会进入到运行期。
泛型类型逻辑上是可以看做是多个不同的类型，实际上都是相同的基本类型。分为泛型类、泛型接口、泛型方法。
**泛型类**
泛型用于类的定义中，被称为泛型类。通过泛型类可以完成对一组类的操作对外开放相同的接口。最典型就是各种容器类，如：List、Set、Map。
```
...
不能对确定的泛型类型使用instanceof操作。如下操作是非法的，编译会出错。
例如：
```
if(object instanceof Generic<number>)
```
instanceof的用法：
...
Result： 布尔类型
Object：任性对象表达式。
Class：必选项。任意已经定义的对象类。
返回值：
返回为Boolean类型。如果Object是Class的一个实例，则返回true，否则返回false。  

**泛型接口**
泛型接口和泛型类的定义和使用基本相同。泛型接口常被用在各种类的生产器中。
```
//定义一个泛型接口
...
```
**泛型通配符**
[泛型通配符的学习参考这篇博客](https://segmentfault.com/a/1190000005337789)

Integer是Number的一个子类，但是Generic<Interger>不是Generic<Number>的子类（即，在使用Generic<Number>作为形参的方法中不能使用Generic<Integer>作为实参传入）。
一言以蔽之：同一种泛型可以对应多种类型，不同的版本的泛型是不兼容的。
类型通配符“?”用于代替未知的具体的类型实参。可以将通配符"?"理解为实参。

通配符的使用可以对泛型参数做出某些限制，使代码更安全，对于**上边界和下边界限定的通配符**总结如下:
